...
You will now be in a **bash** shell in the running container as sudo user **yoctosdk**.       
  
Configure the Yocto scarthgap BSP by running the following scripts in the container:  
...
Note:
* The user/pass for running BSP builds is **yoctosdk/yoctosdk**, although you should not need it (sudo user).   
...
`cd ~/karo-bsp`   
`source setup-environment build-tx93-5210`  
...
After the image is built, it is safe to exit and delete the container. The build directory will persist on the host:  
